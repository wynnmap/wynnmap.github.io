import { hexToRgba, MAP_WIDTH, MAP_HEIGHT, drawOutlinedText, getFadeAlpha, getTerritoryType, hexToRgb, fixHexCode, normalizeTerritoryName } from './utils.js';
...
export function draw(ctx, canvas, mapImage, crownImage, territories, selectedTerritories, guilds, offsetX, offsetY, scale, globalAlpha) {
    ...
    for (const t of territories) {
        ...
        if (prefixAlpha > 0) {
            drawOutlinedText(ctx, t.guild, screenX, textY, "16px Minecraft, monospace", `rgba(255,255,255,${prefixAlpha})`, prefixAlpha);
            textY += 20;
        }
